...
public class SpikeBall extends GameObject {
    ...
    public SpikeBall(int resID, float xPosFlex, float yPosFlex, float lifeTime, boolean isStatic) {
        type = TYPE.SPIKE_BALL;

        if (_sprite == null)
        {
            _sprite = FileSystem.LoadScaledSprite(resID, 1.5f, 1.5f, true);
            size.x = (float) _sprite.getWidth() / 5;
            size.y = (float) _sprite.getHeight();
        }

        _animatedSprite = new AnimatedSprite(_sprite, 1,  8, 24);
